...
res <- subset(res,status %in% c("TO UPLOAD","UPDATE"))
upload.arch <- sprintf("%s_%s.mbtiles",res$map_code,res$map_version)
...
if (nrow(res)!=1 | !file.exists(sprintf("%s/%s",upload.dir,upload.arch))) {
  ...
  # this is not a valid way to set the map to public...
  #  \"private\": false
